...
use polytest_plugin_api::{Result, Transport};
...
impl ChildStdioTransport {
    pub fn from_child(mut child: Child) -> Result<Self> {
        ...
        let stdout = child
            .stdout
            .take()
            .ok_or_else(|| polytest_plugin_api::PluginError::Board("missing stdout".into()))?;
        Ok(Self {
            child,
            stdin,
    ...
    pub fn from_child_stderr(mut child: Child) -> Result<Self> {
        ...
        let stderr = child
            .stderr
            .take()
            .ok_or_else(|| polytest_plugin_api::PluginError::Board("missing stderr".into()))?;
        Ok(Self {
            child,
            stdin,
