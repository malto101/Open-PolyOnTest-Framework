//! PolyTest CLI — composition root (Dependency Inversion).
...
#[derive(Parser, Debug)]
#[command(name = "polytest", version, about = "PolyTest — embedded-first test framework CLI")]
struct Cli {
    #[command(subcommand)]
    cmd: Commands,
}
...
enum Commands {
    /// Run tests using polytest.toml plugin composition
    Run {
        /// Target name from polytest.toml ([target.<name>])
