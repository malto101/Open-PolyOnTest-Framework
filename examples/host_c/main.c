...
POT_SUITE_SETUP(Math) { g_suite_ready = 1; }
POT_SUITE_TEARDOWN(Math) { g_suite_ready = 0; }
POT_SUITE_TAGS(Math, "host", "smoke");

POT_GROUP_SETUP(Math, Basic) { g_group_value = 40; }
POT_GROUP_TEARDOWN(Math, Basic) { g_group_value = 0; }
...
int main(void) { return pot_run_from_env(); }
